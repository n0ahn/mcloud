<script lang="ts">
  ...
  async function handleDelete() {
    ...
    try {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) return;
      await fetch("http://localhost:3000/upload/world", {
        method: "DELETE",
...
<div class="flex flex-col bg-surface border border-surface-border rounded-xl overflow-hidden hover:border-green transition-all duration-200 group">

  <!-- Thumbnail -->
  <div class="relative w-full h-44 bg-bg-primary flex items-center justify-center border-b border-surface-border">
    <svg viewBox="0 0 64 64" class="w-20 h-20 opacity-40 group-hover:opacity-70 transition-opacity" xmlns="http://www.w3.org/2000/svg">
    ...
    <span class="absolute top-3 right-3 text-xs text-text-muted bg-bg-primary border border-surface-border px-2.5 py-1 rounded-md">
      {launcher}
    </span>
  </div>
  ...
  <div class="flex flex-col p-5 gap-4">
    <div>
      ...
      <p class="text-text-muted text-xs mt-1">
        Added {new Date(created_at).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })}
      </p>
    </div>
